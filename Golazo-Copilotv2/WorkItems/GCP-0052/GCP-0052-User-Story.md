...
- **I want:** A documented handoff protocol defining how artifacts and context flow between sequential role subagents, plus end-to-end integration tests that validate the full orchestrator ΓåÆ subagent ΓåÆ artifacts ΓåÆ next-subagent flow
...
- **Assumptions:**
  ...
  - **Assumption (explicit):** Integration tests use mocked subagent execution (simulating file creation) rather than actual LLM calls ΓÇö the tests validate the orchestration machinery, not the LLM output quality
  ...
  - **Assumption (explicit):** A "handoff matrix" table is produced that maps every role pair (N ΓåÆ N+1) to the artifacts that bridge them, making dependency gaps visible

- **Acceptance Criteria (bulleted, testable):**
  - [ ] AC1: `Golazo-Subagent-Handoff-Protocol.md` exists in `WorkItems/` and contains: orchestrator responsibilities, subagent contract, artifact handoff matrix (role ├ù artifacts), error recovery strategy
  - [ ] AC2: The handoff matrix covers all 10 role transitions (including retrospective ΓåÆ POA closure) with specific artifact file patterns for each
...
- **Non-functional requirements:**
  - The handoff protocol document must be concise (Γëñ 200 lines) and structured for quick reference, not exhaustive prose
  - Integration tests must complete in < 10 seconds (no real I/O beyond temp directories)

- **Telemetry / metrics expected:** None

- **Rollout / rollback notes:**
  - Rollout: Merge tests and documentation. No runtime impact ΓÇö this is a verification and documentation work item.
