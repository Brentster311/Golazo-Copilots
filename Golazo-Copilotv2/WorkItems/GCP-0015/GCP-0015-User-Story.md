...
## Assumptions
- **Assumption (explicit)**: Custom roles are defined in a configuration file (e.g., `gcp.yaml` or `.github/gcp-config.yaml`)
- **Assumption (explicit)**: Custom role instructions are loaded from `.github/roles/{custom-role}.md`
- **Assumption (explicit)**: Custom roles are inserted at a specified position in the workflow sequence
...
- Additive feature, no breaking changes to existing workflows
- Workspaces without config file continue with default roles
