...
_USER_AGENT = "GUI/1.0 (KPI Analysis)"
...
_AUTH_HINT_MSG = (
    ...
    "  3. Re-run the analysis \u2014 S360Reporter will reuse Edge\u2019s cookies via CDP."
)
...
def _save_fetched_docs(
    ...
    """Save each fetched document to ``temp/GUI/<kpi_name>/docs/``.

    Returns the docs directory path.
    """
    # Sanitise KPI name for filesystem
    safe_kpi = re.sub(r'[<>:"/\\|?*]', '_', kpi_name)[:80]
    docs_dir = Path(tempfile.gettempdir()) / "S360Reporter" / safe_kpi / "docs"
