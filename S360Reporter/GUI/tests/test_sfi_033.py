...
# Root of S360Reporter package
_SFI_ROOT = Path(__file__).resolve().parent.parent / "src" / "s360_reporter"
...
class TestLLMModuleDeletion:
    ...
    def test_llm_client_import_fails(self):
        """Importing s360_reporter.llm_client must raise ModuleNotFoundError."""
        with pytest.raises(ModuleNotFoundError):
            importlib.import_module("s360_reporter.llm_client")
...
class TestDialogsLLMCleanup:
    ...
    def test_no_configure_llm_dialog(self):
        ...
        import s360_reporter.dialogs as dialogs
...
class TestServicesLLMCleanup:
    ...
    def test_no_load_llm_config_in_services(self):
        ...
        import s360_reporter.services as services
...
class TestLLMAnalysisStub:
    ...
    def test_analyze_with_llm_no_kpi_id_shows_warning(self):
        ...
        with patch("s360_reporter.dialogs.messagebox") as mock_mb:
            from s360_reporter.dialogs import _launch_llm_analysis
            parent = MagicMock()
...
class TestAsyncBridge:
    ...
    def test_async_bridge_starts_background_loop(self):
        ...
        from s360_reporter.copilot_panel import AsyncBridge
...
class TestPyprojectCleanup:
    ...
    def test_no_web_entrypoint(self):
        """s360-reporter-web entry point must be removed."""
        toml_path = Path(__file__).resolve().parent.parent / "pyproject.toml"
        content = toml_path.read_text(encoding="utf-8")
        assert "s360-reporter-web" not in content
