...
- **Assumption (explicit)**: Results will be persisted as JSON files under `%LOCALAPPDATA%/GUI/analyses/` keyed by action item ID — consistent with the durable `s360_client` cache in `%LOCALAPPDATA%`, not the volatile `%TEMP%` user-data cache
...
- [ ] The LLM analysis result is automatically saved to a JSON file on disk under `%LOCALAPPDATA%/GUI/analyses/` keyed by action item ID
