...
### TDD Red Phase
- Wrote 9 test cases in `GUI/tests/test_llm_client.py`:
...
### TDD Green Phase
- Added `fetch_action_item_urls()` to `GUI/src/sfi_reporter/llm_client.py`
- Added helper `_extract_urls()` for URL extraction from 6 fields
- Added `llm-extender>=0.1.0` dependency to `pyproject.toml`
...
| File | Change |
|------|--------|
| `GUI/src/sfi_reporter/llm_client.py` | Added imports (`fetch_url`, `ProviderError`, `re`, `ThreadPoolExecutor`), `_extract_urls()`, `fetch_action_item_urls()` |
| `GUI/src/sfi_reporter/tk_app.py` | Added `fetch_action_item_urls` import, wired URL fetching before `analyze_item()` call |
| `pyproject.toml` | Added `llm-extender>=0.1.0` to dependencies |
| `GUI/tests/test_llm_client.py` | Added 9 SFI-021 test cases |
