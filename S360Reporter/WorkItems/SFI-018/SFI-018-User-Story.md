...
- **So that**: I can launch `S360Reporter.exe` directly without needing to run `az login` first or use the `LAUNCHME.ps1` script
...
- **Assumption (explicit)**: `LAUNCHME.ps1` can be deleted entirely since the app will handle auth. The zip will just contain `S360Reporter.exe` + `README.md`.
