...
Moved org-tree caching from `GUI/services.py` (application layer) into `accia-s360/endpoints/graph.py` (SDK layer) at the `_build_subtree` method level.
...
### GUI/src/sfi_reporter/services.py
...
- S360Reporter (core): 21 passed, 5 errors (pre-existing Azure CLI issues in `test_data.py`)
