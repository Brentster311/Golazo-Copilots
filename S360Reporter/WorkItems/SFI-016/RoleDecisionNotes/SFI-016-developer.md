...
| File | Change |
|---|---|
| `GUI/src/sfi_reporter/data.py` | Singleton `get_client()`, `get_detailed_action_items` → tuple return, `failed_kpis` tracking |
| `GUI/src/sfi_reporter/tk_app.py` | Retry button, `_on_retry_failed()`, `_on_retry_complete()`, updated `_on_refresh_complete()` |
| `GUI/tests/test_data.py` | Autouse fixture to reset `_client_instance` singleton |
| `GUI/tests/test_tk_app.py` | Updated mock return values from `[]` to `([], [])` |
