...
### API Contract
- **Auto-detect**: `discover_azure_configs(*, subscription_id=None, api_version=None) -> list[LLMConfig]` — returns `llm_extender.config.LLMConfig` objects. The dialog needs to map these to S360Reporter's own `sfi_reporter.llm_client.LLMConfig` fields: `base_url` → `endpoint`, `deployment` → `deployment`, `model` → display only, `api_version` → `api_version`.
...
- **Auto-detect credentials**: `discover_azure_configs()` uses `AzureCliCredential` from the user's `az login` session. No credentials flow through S360Reporter code.
