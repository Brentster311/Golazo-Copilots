...
Add a "Configure LLM" button to the S360Reporter main screen that opens a modal dialog where the user can manually enter Azure OpenAI connection details (endpoint, deployment, API version) or click "Auto-detect" to discover available configurations from their Azure CLI credentials via `llm_extender.discover_azure_configs()`. The chosen config is persisted to `settings.json` and used for all subsequent LLM analysis calls.
...
- S360Reporter end users (developers managing SFI remediation items)
