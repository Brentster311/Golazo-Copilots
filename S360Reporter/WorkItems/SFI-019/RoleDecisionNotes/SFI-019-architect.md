...
| BD-4 | `eta_logic.py` in S360Reporter package, not in `accia-s360` | `GUI/src/sfi_reporter/` |
...
| `GUI/src/sfi_reporter/eta_logic.py` | New | `propose_eta()`, `get_items_needing_eta_update()`, `build_eta_update()` |
| `GUI/src/sfi_reporter/tk_app.py` | Modify | Add button, dialog classes, wire into `ItemDetailsModal` |
| `accia-s360/tests/test_eta_payload.py` | New | Tests for updated payload format (TC-06, TC-07, TC-08) |
| `GUI/tests/test_eta_logic.py` | New | Tests for eta_logic functions (TC-01 through TC-05) |
| `GUI/tests/test_eta_ui.py` | New | Tests for UI flow/validation (TC-09 through TC-15) |
