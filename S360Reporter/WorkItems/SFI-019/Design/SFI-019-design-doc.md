...
Add the ability to update ETA dates and user statuses for SFI action items directly from the S360Reporter tkinter app. Two entry points: (1) a main-screen button for bulk updates of all invalid-ETA items, and (2) an individual "Update ETA" button in the detail view.

## Problem Statement

Users currently must open the S360 web portal or run a separate Sauron script to update ETAs. This is friction-heavy: the portal requires navigating to each item individually, and the Sauron script is a separate tool with its own auth flow. S360Reporter already displays all the data needed — it just can't write back.

## Business Case

- **Why now**: S360Reporter is the daily tool for SFI triage; adding write-back closes the loop in a single app
...
- **S360Reporter maintainer**: Brent (sole developer)
...
Create `GUI/src/sfi_reporter/eta_logic.py`:
