...
- **Assumptions**:
  - **Assumption (explicit)**: Cache file stored in the existing `get_cache_dir()` directory (`tempdir/GUI/`) using a separate file from the main user-data cache (e.g., `{alias}_org_tree.json`). This avoids coupling with the main 1-hour cache lifecycle.
