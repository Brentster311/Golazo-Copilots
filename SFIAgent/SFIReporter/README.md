...
Download `SFIReporter.zip`, extract, and run **SFIReporter.exe**. No Python installation required.
...
- **Score/Cost table enhancements**: Services, Programs, and KPIs tables display `Score` before `Cost (min)` and include a derived `Score/Min` column. For incoming zero-cost rows, displayed cost falls back to `28,800` minutes and ratio uses that fallback.
