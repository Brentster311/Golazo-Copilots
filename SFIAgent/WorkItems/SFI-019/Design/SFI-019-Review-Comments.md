...
**Finding**: The Sauron payload includes `"AssignedTo": alias` in each ActionItem. The design doesn't specify where this comes from in the SFI Reporter context.
...
`eta_logic.py` goes in `SFIReporter/src/sfi_reporter/eta_logic.py` — it is UI-layer logic (ETA proposal, item filtering) not a client-library concern.

### Binding Decision BD-5: No Changes to `s360_client` Package

Only `accia-s360` is modified (the package SFI Reporter actually imports). The parallel `src/s360_client/` package is not touched — it's a separate project with its own tests.
