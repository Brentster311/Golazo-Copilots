...
LLMExtender's `LLMConfig` has `base_url`, `model`, `deployment`. SFIReporter's `LLMConfig` has `endpoint`, `deployment`, `api_key`. The dialog maps `base_url → endpoint` and `deployment → deployment` when populating from auto-detect. This is a display-layer concern only.
