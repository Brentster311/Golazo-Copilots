...
3. **LLMExtender → SFIReporter config mapping**: `discover_azure_configs()` returns `llm_extender.config.LLMConfig` with `base_url`. The dialog maps `base_url → endpoint` when populating fields. The combo display shows `endpoint — deployment (model)`.
