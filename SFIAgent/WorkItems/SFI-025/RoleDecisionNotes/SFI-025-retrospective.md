...
- **LLM-0012 review paid off**: Reviewing the LLMExtender discovery feature beforehand meant the integration mapping (LLMExtender config → SFIReporter config) was understood upfront with no surprises.
...
- **Two LLMConfig types**: SFIReporter's `LLMConfig` and LLMExtender's `LLMConfig` have different field names (`endpoint` vs `base_url`). This is a minor confusion point. Future consolidation could help.
