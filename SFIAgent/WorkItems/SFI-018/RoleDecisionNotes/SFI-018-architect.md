...
### Scope Containment
- Change is isolated to `accia-s360/src/accia_s360/auth.py` (credential creation) + SFI Reporter cleanup (LAUNCHME.ps1 removal, docs)
...
2. **`s360_client/auth.py` untouched** — it's the older standalone package, not used by SFI Reporter. Updating it would be scope creep.
