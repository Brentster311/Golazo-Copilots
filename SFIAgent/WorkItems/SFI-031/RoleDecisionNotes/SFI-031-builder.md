...
## Build Verification
- `pytest SFIReporter/tests/ -m "not live"`: **254 passed**, 19 pre-existing errors, 0 new failures
- No build/packaging step required (library code, not a standalone build)

## Files Changed
- `SFIReporter/src/sfi_reporter/services.py` — production code (cache helpers + modified `get_org_mapping`)
- `SFIReporter/tests/test_sfi_031.py` — 11 new tests
