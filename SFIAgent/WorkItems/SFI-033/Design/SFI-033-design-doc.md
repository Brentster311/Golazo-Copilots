...
## Summary
Replace the "⚙️ Configure LLM" toolbar button and Azure OpenAI-based analysis workflow with a GitHub Copilot SDK-powered chat side panel. The side panel appears on the right side of the SFI Reporter window when the user clicks "open"/"LLM" buttons, and can be closed via its own X button. The existing "Analyze with LLM" right-click menu items are preserved but stubbed to show "Not yet implemented".
...
## Stakeholders
- SFI Reporter end users (managers and ICs)
- SFI Reporter developer (Brent)
...
1. Delete `SFIReporter/src/sfi_reporter/llm_client.py`
2. Delete `SFIReporter/src/sfi_reporter/llm_storage.py`
3. Delete `SFIReporter/tests/test_llm_client.py` and `SFIReporter/tests/test_llm_storage.py`
4. Delete `SFIReporter/tests/test_sfi_025.py` (ConfigureLLMDialog tests)
...
| File | Reason |
|---|---|
| `SFIReporter/src/sfi_reporter/llm_client.py` | Azure OpenAI client — replaced by Copilot SDK |
| `SFIReporter/src/sfi_reporter/llm_storage.py` | Analysis result storage — no longer needed |
| `SFIReporter/tests/test_llm_client.py` | Tests for deleted module |
| `SFIReporter/tests/test_llm_storage.py` | Tests for deleted module |
| `SFIReporter/tests/test_sfi_025.py` | Tests for ConfigureLLMDialog |
| `SFIReporter/docs/analyze-with-llm.md` | Documentation for removed feature |

## Files to Modify
| File | Changes |
|---|---|
| `SFIReporter/src/sfi_reporter/app.py` | Remove ConfigureLLMDialog import/button; add open/LLM buttons + CopilotPanel |
| `SFIReporter/src/sfi_reporter/dialogs.py` | Remove ConfigureLLMDialog, AnalysisModal, AnalysisProgressModal, analysis helpers; rewrite `_launch_llm_analysis` as stub |
| `SFIReporter/src/sfi_reporter/services.py` | Remove `_load_llm_config()` and its `__all__` entry |
| `SFIReporter/pyproject.toml` | Remove llm-extender dep; add github-copilot-sdk |
