...
- **So that**: consumers (like SFIReporter) can accurately map any user's org tree without relying on S360's limited hierarchy APIs which only return shallow team IDs

- **Out of scope**:
  - Changes to SFIReporter's `tk_app.py` or any consumer code (that's SFI-026)
