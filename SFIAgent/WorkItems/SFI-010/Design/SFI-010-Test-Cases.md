...
def test_column_cache_path():
    """Column metadata cache is at $TEMP/sfireporter/column_metadata.json"""
    from sfi_reporter.data import get_column_cache_path
    import os
    path = get_column_cache_path()
    assert path.endswith("sfireporter/column_metadata.json") or \
           path.endswith("sfireporter\\column_metadata.json")
```
