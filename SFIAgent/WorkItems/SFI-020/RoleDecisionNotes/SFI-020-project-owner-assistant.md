...
### Interface Type
- **GUI (tkinter)** — confirmed by the user's request ("right click a KPI row"), which is the existing SFIReporter desktop app.

### Target Platform
- **Windows** — consistent with all prior SFI work items and the existing `.spec` PyInstaller build.

### Data Persistence
- **JSON files on disk** under `%LOCALAPPDATA%/sfireporter/analyses/`, keyed by action item ID.
