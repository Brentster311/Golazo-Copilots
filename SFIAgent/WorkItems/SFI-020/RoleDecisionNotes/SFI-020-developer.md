...
| File | Purpose |
|------|---------|
| `SFIReporter/src/sfi_reporter/llm_client.py` | LLMConfig, build_prompt, analyze_item, AnalysisResult, _parse_sections |
| `SFIReporter/src/sfi_reporter/llm_storage.py` | save_analysis, load_analysis, analysis_exists (atomic writes, %LOCALAPPDATA%) |
| `SFIReporter/tests/test_llm_client.py` | 23 tests — config, prompt, analyze, parsing, truncation, security |
| `SFIReporter/tests/test_llm_storage.py` | 10 tests — save, load, exists, atomic write, corruption, sanitization |

### Files Modified
| File | Changes |
|------|---------|
| `SFIReporter/src/sfi_reporter/tk_app.py` | `<Button-3>` bindings on `tree_kpis` and `DetailModal` treeview; context menu handler; `AnalysisProgressModal`; `AnalysisModal`; `_analyze_with_llm()`, `_on_analysis_complete()`, `_on_analysis_error()` functions |
| `SFIReporter/pyproject.toml` | Added `openai>=1.0.0` dependency |
...
- **180 total SFIReporter tests passed** (zero regressions)
