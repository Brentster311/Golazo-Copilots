...
10. **Clause caching via existing cache directory**: Clauses are persisted to `%TEMP%/sfireporter/query_clauses.json` using the same cache directory as the existing data cache (`cache.py:get_cache_dir()`). JSON format — simple list of `{connector, field, operator, value}` dicts. Auto-restored on form open, auto-saved on Run Query. Clear button removes the cache file. No save/load UX needed.
