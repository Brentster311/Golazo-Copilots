...
Add a clause-based query builder window (inspired by ADO/IcM query editor) to SFI Reporter. Users build ad-hoc queries by adding `(And/Or, Field, Operator, Value)` clauses against any column in the loaded action item data. Results show a per-program summary (Total / Out of SLA / Invalid ETA) with drill-down into filtered items.
...
### FR-7: Clause Caching
- On "Run Query", serialize current clauses to `%TEMP%/sfireporter/query_clauses.json`
...
Create a new file `SFIReporter/src/sfi_reporter/query_builder.py` containing:
...
- `%TEMP%/sfireporter/query_clauses.json` — uses existing `get_cache_dir()` from `cache.py`
