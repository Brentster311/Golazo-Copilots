...
- **As a**: SFI lead / manager using the SFI Reporter desktop app
...
- **Assumption (explicit)**: The current set of query clauses is cached to `%TEMP%/sfireporter/` (same cache directory as existing data cache) as a JSON file. On form open, the last-used clauses are restored automatically. No explicit save/load UI needed.
...
- Query clauses cached as JSON in `%TEMP%/sfireporter/query_clauses.json` — survives app close/reopen with no user action
