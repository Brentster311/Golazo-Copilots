...
from ..core.output_validator import parse_required_outputs
...
from ..roles.loader import get_role_content, load_role_instructions
from .status_helpers import (
    apply_closure_completion_override,
    build_deviations,
    build_version_warning,
    find_missing_role_notes,
...
async def golazo_status(
    ...
    async def _async_validate_outputs():
        ...
        return await asyncio.to_thread(validate_required_outputs, output_specs, workspace_root)

    async def _async_check_missing_notes():
        """Check for missing role notes in a thread."""
        return await asyncio.to_thread(find_missing_role_notes, state, work_item_id, work_items_dir)
    ...
    unwrapped = unwrap_parallel_results(
        output_result=output_result,
        missing_notes_result=missing_notes_result,
        stale_files_result=stale_files_result,
        registry_result=registry_result,
        progress_result=progress_result,
        state=state,
    )
    outputs_complete = unwrapped["outputs_complete"]
    required_outputs = unwrapped["required_outputs"]
    missing_notes = unwrapped["missing_notes"]
    stale_files = unwrapped["stale_files"]
    registry_hint = unwrapped["registry_hint"]
    role_progress = apply_closure_completion_override(
        role_progress=unwrapped["role_progress"],
    ...
    deviations = build_deviations(state.deviations)
    
    # GCP-0037: Per-file stale version reporting
    version_warning = build_version_warning(stale_files)
