...
Role execution policy (authoritative):
...
- **Never** ask the user questions when executing as a subagent (make documented assumptions)
...
3. Orchestrator performs the role inline for the current role and remains inline for the rest of the session.
...
3. Continues inline for all remaining roles in the session unless the user explicitly re-enables subagents.
...
Subagent Rules:
  ...
  ✓ Follow role instructions   ✗ Ask user questions (when executing as subagent)
